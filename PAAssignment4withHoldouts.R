...
# Holdout Sets #########
holdout_data <- read.csv("insurance_data(1).csv")

#remove the same columns as earlier
...
#This creates a response predictor variable with all 0s.
holdout_data$response<-0

#Creating case holdout probabilities
case_holdoutprob<- predict(model_gbm, holdout_data, type="prob")

#Creating scored case holdouts
case_holdout_scored<- cbind(holdout_data, case_holdoutprob$buy)
case_holdout_scored[1:5,]


# Printing/Viewing case holdouts
print(case_holdout_scored)
view(case_holdout_scored)

#install.packages("openxlsx")
library(openxlsx)

#Save holdout File
write.xlsx(case_holdout_scored, "C:/Users/catho/OneDrive/Desktop/case_holdout_data.xlsx")
